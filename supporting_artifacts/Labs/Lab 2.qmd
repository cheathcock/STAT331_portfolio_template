...
The dimensions of this data is 30463 rows by 15 columns. (code in code chunk 1)

**3. What are the data types of the variables in the dataset?**

The data types of the variables in the dataset are shown above. There is numerical variables, charcter variables, as well as 1 data variable.
...
ggplot(data = surveys, 
       mapping = aes(y = species, x = weight)) + 
  geom_boxplot() + 
  labs(x = "Weight (g)", y = "Species of Animal")
```
...
ggplot(data = surveys, 
  ...
  geom_boxplot() + 
  labs(y = "Species of Animal", x = "Weight (g)") 
```
...
colorpalette <- c("#fe3093", "#f7f926", "#c8fce5", "#479b55", "#d63cff", "#11fa35", "#fb6955", "#c8fce5")
  
surveys |> 
  ggplot(aes(y = species, x = weight, color = genus)) +
  geom_boxplot() +
